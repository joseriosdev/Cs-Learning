...
namespace TinyACTIO_RESTful.Utilities
{
    public class DBMethods
    {
        ...
        public static void InsertEvent(Event evt)
        {
            using (SqlConnection connection = new SqlConnection(_connectionStr))
            {
                string sql = $"INSERT EVENTS ([Name],[Organized_By],[Workspace_Id],[Created_Date]) VALUES ('{evt.Name}',1,1,'{evt.CreatedDate}')";
                SqlCommand sqlCommand = new SqlCommand(sql, connection);
                try
                {
                    connection.Open();
        ...
        public static Event GetSingleEvent(int id)
        {
            using (SqlConnection connection = new SqlConnection(_connectionStr))
            {
                string sql = $"SELECT Id,Name,Created_Date,Workspace_Id,Organized_By FROM EVENTS WHERE Id={id}";
                SqlCommand sqlCommand = new SqlCommand(sql, connection);
                try
                {
                    connection.Open();
                    ...
                    evt.Id = reader.GetInt32(0);
                    evt.Name = reader.GetString(1);
                    evt.CreatedDate = reader.GetDateTime(2);
                    evt.WorkSpaceId = reader.GetInt32(3);
                    evt.OrganizedById = reader.GetInt32(4);
                    reader.Close();
                    connection.Close();
                    return evt;
                }
